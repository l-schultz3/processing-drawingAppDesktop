...
void setup() {
  ...
  if (height / 1.75 - 1 > 255 + 2 * redLineY) {
    controlHeight = height / 1.75 - 1;
  } else {
    controlHeight = redLineY + 255;
  }
}
...
void drawControls() {
  ...
  fill(red, green, blue);
  ellipse(width * 0.78, controlHeight / 4, size, size);
